//keypair da cui parte il trasferimento, connection, systemprogram infatti si usa system transfer per il transferimento,
//trnasaction (dentro contiene istruzione del trasferimento) e sendAndConfirmation (API aspetta finche transazione non va a buon fine)
import { 
        Keypair, 
        Connection, 
        LAMPORTS_PER_SOL,
    ...
    const to = new PublicKey("8LmQiqF7NNTscsmyfza6f2BVJC3WVq4uAAKixR6cJcL6");
    ...
    */
    ...
                fromPubkey: from.publicKey,             // Indirizzo del wallet mittente (deve firmare la transazione) (accede la public key di quella keypair)
    ...
            // Posso agigungere altre istruzioni con .add(transferInstruction2)
    ...
            // chiede: connessione, transazione o istruzioni, array di firme (infatti posso avere + persone che firmano inoltre se non metto il fee payer, prende il primo firmatario come fee pyaer)
            //         posso cambiare il cimmitment, skipPreFlight = true permette di vedere il fallimento della transazione on chain (transazione eseguita e si blocca quando commette l'errore)
            // OSS: si può nascondere keypair all'interno secret del sito e lo faccio firmare lui cosi la keypair del protocollo paga la fee
